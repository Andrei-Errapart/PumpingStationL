...
namespace PlcCommunication
{
    public class IOSignal : DependencyObject
    {
        ...
        /// </summary>
        public const string SRS = "/"; // 'scope resolution' separator mark (SRS), to separate PLC name and signal name
        ...
        public string FqName { get { return _FqName; } }
        ...
        /// <summary>
        /// Integer value of the signal. Use Update to update the value.
        /// </summary>
        public int Value { get { return _Value; } }
        private int _Value;
        /// <summary>
        /// Value of the signal as real number. Use Update to update the value.
        /// </summary>
        ...
        public IOSignal(int Id, string Name, bool IsVariable, TYPE Type, string Device, string IOIndex, string Description, string Text0, string Text1, string PlcName = "")
        {
            this._UID = _NextUID++;
            this._Id = Id;
            this._Name = Name;
            this._PlcName = PlcName;
            this._FqName = PlcName.Length > 0 ? PlcName + SRS + Name : "";
            this._IsVariable = IsVariable;
            this.Type = Type;
            this._CanWrite = Type == TYPE.COIL || Type == TYPE.HOLDING_REGISTER;
            this.BitCount = Type==TYPE.REGISTER32 ? 32 :  ((Type == TYPE.DISCRETE_INPUT || Type == TYPE.COIL) ? 1 : 16);
            this._Device = Device;
        ...
        public void UpdateValueOnly(Tuple<bool, int> Value)
        {
            ...
            this._RealValue = (float) Value.Item2;
        }

        public static TYPE? TypeOfString(string s)
        {
            if (string.Equals(s, "input", StringComparison.InvariantCultureIgnoreCase))
            {
                return TYPE.DISCRETE_INPUT;
            }
            if (string.Equals(s, "output", StringComparison.InvariantCultureIgnoreCase))
            {
                return TYPE.COIL;
            }
            if (string.Equals(s, "input_register", StringComparison.InvariantCultureIgnoreCase))
            {
                return TYPE.INPUT_REGISTER;
            }
            if (string.Equals(s, "holding_register", StringComparison.InvariantCultureIgnoreCase))
            {
                return TYPE.HOLDING_REGISTER;
            }
            if (string.Equals(s, "register32", StringComparison.InvariantCultureIgnoreCase))
            {
                return TYPE.REGISTER32;
            }
            return null;
        }
